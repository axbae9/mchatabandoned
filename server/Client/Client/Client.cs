...
namespace Client
{
    public partial class Client : Form
    {
            ...
            if (!exit)
            {
                connectButton.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (status)
                    {
                        ...
                        Log(SystemMsg(addrTextBox.Text + " Connected!"));
                    }
                    else
                    {
                        ...
                        Log(SystemMsg(addrTextBox.Text + " Disconnected"));
                    }
                });
            }
        }
        ...
        private bool Authorize()
        {
            ...
            if (!connected)
            {
                Log(SystemMsg("Password might be wrong."));
            }
            return success;
        }
        ...
        private void ConnectButton_Click(object sender, EventArgs e)
        {
            ...
            else if (client == null || !client.IsAlive)
            {
                ...
                else
                {
                    ...
                    {
                        error = true;
                        Log(SystemMsg("IP Address Problem A1"));
                    }
                }
                int port = -1;
                if (number.Length < 1)
                ...
                {
                    error = true;
                    Log(SystemMsg("Port Problem A1"));
                }
                if (username.Length < 3)
                {
                    error = true;
                    Log(SystemMsg("You Should Change Your Name"));
                }
                if (!error)
                {
        ...
        // design stuff
        {
            ...
            this.connectButton.BackColor = System.Drawing.Color.FromArgb(114, 137, 218);
            this.logTextBox.BackColor = System.Drawing.Color.FromArgb(153, 170, 181);
            this.sendTextBox.BackColor = System.Drawing.Color.FromArgb(153, 170, 181);
        }
    }
}
