...
namespace Server
{
    public partial class Server : Form
    {
        ...
        {
            return string.Format("Server: {0}", msg);
        }
            ...
            if (!exit)
            {
                startButton.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (status)
                    {
                        ...
                        Log(SystemMsg("Server is Online!"));
                    }
                    else
                    {
                        ...
                        Log(SystemMsg("Server is Offline"));
                    }
                });
            }
        }
        ...
        private void AddToGrid(long id, string name)
        {
            if (!exit)
            {
                clientsDataGridView.Invoke((MethodInvoker)delegate
                {
                    ...
                    totalLabel.Text = string.Format("Active User Count: {0}", clientsDataGridView.Rows.Count);
                });
            }
        }
        ...
        private void Connection(MyClient obj)
        {
            if (Authorize(obj))
            {
                ...
                string msg = string.Format("[ + ] {0}", obj.username);
                Log(SystemMsg(msg));
                ...
                msg = string.Format("[ - ] {0}", tmp.username);
                Log(SystemMsg(msg));
                Send(msg, tmp.id);
            }
        }
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            else if (listener == null || !listener.IsAlive)
            {
                ...
                {
                    error = true;
                    Log(SystemMsg("IP Address is null..."));
                }
                else
                {
                    try
                    ...
                    {
                        error = true;
                        MessageBox.Show("IP Address did not respond", "mChat");
                    }
                }
                int port = -1;
                if (number.Length < 1)
                {
                    error = true;
                    MessageBox.Show("Port can't be null...", "mChat");
                }
                else if (!int.TryParse(number, out port))
                {
                    error = true;
                    MessageBox.Show("Port Problem A1", "mChat");
                }
                else if (port < 0 || port > 65535)
                {
                    error = true;
                    MessageBox.Show("Port Problem B1", "mChat");
                }
                if (username.Length < 1)
                {
                    error = true;
                    MessageBox.Show("You should change your username", "mChat");
                }
                if (!error)
                {
                    ...
                    listener.Start();
                    this.clientsDataGridView.Visible = true;
        ...
        private void SendTextBox_KeyDown(object sender, KeyEventArgs e) // sending message as server
        {
        ...
        private void Server_Load(object sender, EventArgs e)
        {
            ...
            this.startButton.BackColor = System.Drawing.Color.FromArgb(114, 137, 218);
            this.disconnectButton.BackColor = System.Drawing.Color.FromArgb(114, 137, 218);
            this.clearButton.BackColor = System.Drawing.Color.FromArgb(114, 137, 218);
            this.logTextBox.BackColor = System.Drawing.Color.FromArgb(153, 170, 181);
            this.clientsDataGridView.BackgroundColor = System.Drawing.Color.FromArgb(153, 170, 181);
